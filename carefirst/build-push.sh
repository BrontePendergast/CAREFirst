...
if [[ "$VERBOSE" = true ]]; then
    ...
    echo " -- Build and push ${IMAGE_NAME}:${TAG} to ${ACR_DOMAIN}/${IMAGE_PREFIX}/${IMAGE_NAME}"
    az acr login --name w255mids &> /dev/null
    docker build --platform linux/amd64 -t ${IMAGE_NAME}:${TAG} . &> /dev/null
    docker tag ${IMAGE_NAME}:${TAG} ${IMAGE_FQDN}:${TAG} &> /dev/null
    docker push ${IMAGE_FQDN}:${TAG} &> /dev/null


    echo " -- Deploy ${IMAGE_NAME}:${TAG} on AKS"
    kustomize build .k8s/overlays/prod | kubectl apply -f - &> /dev/null
fi 
