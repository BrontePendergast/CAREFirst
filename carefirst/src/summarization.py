...
question_system_prompt = SystemMessagePromptTemplate.from_template(
    template="""
    Given the chat history and the current message, rephrase the current message to be a standalone message.
    ...
    This standalone message will match one of the following tags of the type of message:
    - "Prevention" 
    - "Care" (what to do)
    - "Call"
    - "Signs and symptoms"
    ...
    Respond with the standalone message and associated tag and keywords in JSON format
    {format_instructions}
    """)


question_human_prompt = HumanMessagePromptTemplate.from_template(
    template = "Chat history: \n{chat_history}\n Human Message: \n {question}\n please rewrite the human message and provide the tag", 
